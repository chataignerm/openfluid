...
: QWizard()
{
  ...
  QString IconPath = QString::fromStdString(openfluid::base::RuntimeEnvironment::getInstance()->getCommonResourceFilePath("openfluid_icon.png"));
...
void MarketClientAssistant::setupDownloadPage()
{
  ...
  mp_InstallTreeView = new QTableWidget(0, 4);

  QStringList ColumnNames;
  ColumnNames << tr("Package ID") << tr("Type") << tr("Format") << tr("Status");
  mp_InstallTreeView->setHorizontalHeaderLabels(ColumnNames);
  mp_InstallTreeView->horizontalHeader()->setStretchLastSection(true);
  mp_InstallTreeView->verticalHeader()->setVisible(false);


  m_InstallSWindow.setWidget(mp_InstallTreeView);
...
void MarketClientAssistant::onInstallTimeoutOnce()
{
  ...
  {
    QTableWidgetItem *StatusItem = mp_InstallTreeView->item(InstalledCount,
        InstallTreeViewColumns::STATUS);
    mp_InstallTreeView->scrollToItem(StatusItem);
...
void MarketClientAssistant::updateInstallTreeview()
{
  ...
  {
    int TmpRow = mp_InstallTreeView->rowCount();
    mp_InstallTreeView->insertRow(TmpRow);
    ...
    mp_InstallTreeView->setItem(TmpRow, 0, m_InstallColumns.mp_ID);
    mp_InstallTreeView->setItem(TmpRow, 1, m_InstallColumns.mp_Type);
    mp_InstallTreeView->setItem(TmpRow, 2, m_InstallColumns.mp_Format);
    mp_InstallTreeView->setItem(TmpRow, 3, m_InstallColumns.mp_Status);
  }

  mp_InstallTreeView->resizeColumnsToContents();
}
